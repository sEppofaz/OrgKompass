...
## Modul-Scroll-Position übersteht Tab-Wechsel und App-Neustart (seit 2026-08-15, v2.9)

- **Problem:** Scroll-Position innerhalb eines Moduls ging verloren, sobald man zu einem anderen Tab wechselte (z.B. Notizen) oder die App verließ/neu startete — man landete immer wieder am Modulanfang statt an der zuletzt gelesenen Stelle.
- **Persistenz:** neuer `localStorage`-Key `ok_module_state` (`{moduleId, scrollTop}`), einziger Eintrag (kein Verlauf pro Modul). `saveLastModuleState()`/`getLastModuleState()`/`clearLastModuleState()`.
- **Speichern:** `main`-Scroll-Listener (in `setupScrollToTop()` erweitert, gemeinsam mit dem Scroll-to-Top-Button) speichert bei jedem Scroll im Lernen-Tab mit offenem Modul, `requestAnimationFrame`-gedrosselt (ein Schreibvorgang pro Frame statt pro Scroll-Event).
- **Wiederherstellen:** zweimal — (1) `switchTab()`: beim Wechsel zurück zu Lernen mit noch gesetztem `STATE.activeModuleId`, `main.scrollTo` auf den gespeicherten Wert (nur falls `moduleId` übereinstimmt, sonst 0). (2) `init()`: beim App-Start, falls `ok_module_state` einen Eintrag hat, wird `STATE.activeModuleId` daraus vorbelegt und nach dem ersten Render die Scroll-Position gesetzt.
- **Explizit gelöscht** (bewusste „Home"-Gesten, kein Wiedereinstieg gewollt): Klick auf „zurück" (`[data-back-to-modules]`), Pull-to-Refresh, erneutes Antippen des bereits aktiven Lernen-Tabs (s.o.).
- **Kein Rollout-Standard bisher** — nur in OrgKompass umgesetzt.
...
**Verfeinerung seit v2.9:** Die obige Regel gilt jetzt nur noch für das **erneute Antippen des bereits aktiven** Lernen-Tabs (Original-Bugfall). Ein Wechsel **weg von** Lernen zu einem anderen Tab setzt `STATE.activeModuleId` nicht mehr zurück — Josef wollte beim Zurückwechseln (oder nach App-Neustart) an der zuletzt gelesenen Stelle im Modul weitermachen, nicht an der Modulübersicht. Siehe eigener Abschnitt „Modul-Scroll-Position übersteht Tab-Wechsel und App-Neustart" unten.
